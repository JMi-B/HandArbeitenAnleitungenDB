...
def AnleitungInsert(thisrow,thismedium_id):
    mycursor.execute(
...
def AnleitungExist(thisrow):
    mycursor.execute(
        "SELECT * FROM  anleitung WHERE NameAnleitung =%s AND Seitenzahl = %s;",
        (
...
for daten, row in df.iterrows():
    ...
    CheckMedienArt(medienart,row,titelmedium)
    ...
    anleitung_id = AnleitungExist(row)
    if anleitung_id < 1:
        medium_id = CheckMedienArt(medienart,row,titelmedium)
        print(medium_id)
        anleitung_id = AnleitungInsert(row,medium_id)
